...
safe. Tested by tests/test_hook.py.
"""
import json
import os
import re
...
def active_state(project):
    """(root, slug, state_text) for the active feature, following one
    `worktree:` pointer; None when no feature is active/readable.

    root is the feature's WORKING root — the worktree when a pointer is
    followed, else `project`. path checks resolve against it, so a worktree's
    source (which lives under the main checkout's .claude/worktrees/) is
    judged as source, not masked as writable .claude/ config."""
...
def decide(data, project):
    ...
    if tool == "Bash":
        reason = bash_verdict(tool_input.get("command") or "", root)
        if reason is None:
            return 0, None
        detail = f"Blocked Bash command: {reason}."
    else:
        path = tool_input.get("file_path") or tool_input.get("notebook_path")
        if path_allowed(path, root):
            return 0, None
