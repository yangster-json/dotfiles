...
class TestFileTools(HookProject):
    ...
    def test_worktree_pointer_followed(self):
        # the feature lives in a worktree under the main checkout's
        # .claude/worktrees/, where the real source edits happen. path checks
        # must resolve against the worktree root: its source is gated, its own
        # specs/ stays writable (the pipeline writes state there), and the gate
        # keys off the worktree's state.md.
        wt = ".claude/worktrees/sdd/feat"
        self.write("specs/ACTIVE", f"worktree: {wt}\n")
        self.write(f"{wt}/specs/ACTIVE", "feat\n")
        self.write(f"{wt}/specs/feat/state.md", STATE_UNAPPROVED)
        src = self.in_root(f"{wt}/src/main.c")
        spec = self.in_root(f"{wt}/specs/feat/spec.md")
        # source inside the worktree is source, not writable .claude/ config
        self.assertFalse(self.allowed(edit(src)))
        # ...but the worktree's own specs/ must stay writable pre-approval
        self.assertTrue(self.allowed(edit(spec)))
        self.write(f"{wt}/specs/feat/state.md", STATE_APPROVED)
        self.assertTrue(self.allowed(edit(src)))
