...
used by ~/.claude/sdd/watch, statusline-segment.py, quality, and the
plan-approval hook. state.md is written by an LLM orchestrator, so nothing
here may depend on exact spacing, column order, emphasis, or case — parse
by structure (headers, anchored keys), never by position.

tested by ~/.claude/sdd/tests/test_statelib.py.
"""
import os
import re
